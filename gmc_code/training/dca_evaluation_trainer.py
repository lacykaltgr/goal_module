...
class DCAEvaluator(LightningModule):
    def __init__(
        ...
        # Store test outputs for epoch end processing
        self.test_outputs = []
    ...
    def test_step(self, batch, batch_idx):
        ...
        if self.scenario == 'dummy_goal':
            # For dummy_goal: features, goal_features, actions, goal_position, dataset_index
            # We'll use the first 4 components for the model
        ...
        # Store outputs for epoch end processing in PyTorch Lightning 2.0
        self.test_outputs.append(output_dict)
        return output_dict
